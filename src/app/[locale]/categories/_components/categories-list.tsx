    ...
    return (
        <div className='py-4 gap-[60px] grid grid-cols-3' dir='ltr'>
            {categories.map((category) => (
                <div key={category._id} className='px-2.5 py-4  hover:border-b-2 relative hover:border-softpink-300 hover:bg-gradient-to-t from-softpink-300/25 to-transparent transition-all duration-300'>
                    <Link href={`/categories/${category._id}`} className='gap-2.5 col-span-1 flex '>
                        {/*category*/}
                        <h3 className='text-8xl font-great text-softpink-200 absolute top-1/2 start-1/2 transform -translate-x-1/2 -translate-y-1/2 z-0 dark:text-zinc-600'>Category</h3>
                        {/* image*/}
                        <Image src={img} alt='gift image' width={109.47} height={111} className='h-[111px] z-10' />
                        {/*content */}
                        <div className='py-5 z-20'>
                            {/*name*/}
                            <h3 className='text-maroon-700 text-4xl font-semibold capitalize dark:text-softpink-300'>{category.name}</h3>
                            {/*productsCount*/}
