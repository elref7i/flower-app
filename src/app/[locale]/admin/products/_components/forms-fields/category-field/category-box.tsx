...
import useGetInfiniteCategories from "./hook/use-get-infinite-categories";

// Combobox for category
export function CategoryBox({ setBoxValue }: { setBoxValue: (value: string) => void }) {
  // hooks
  const [open, setOpen] = React.useState(false);
  const [value, setValue] = React.useState("");
  const loadMore = React.useRef<HTMLInputElement | null>(null);
  const t = useTranslations("products-cu");
  const { categories, fetchNextPage, hasNextPage, isFetchingNextPage } = useGetInfiniteCategories();

  // generate label and value
  const frameworks = categories.map((cat) => {
    return { label: cat.name, value: cat._id };
  });

  // add observer to fetch next categories
  React.useEffect(() => {
