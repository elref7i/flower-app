...
export default function UpdateForm({ address }: { address: Address }) {
  //variables
  ...
  const { mutateAsync, isPending } = useMutation({
    mutationFn: ({ id, values }: { id: string; values: FormData }) => updateAddress(id, values),
    onSuccess: () => {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        {step === 0 && (
          <>
            {/*title*/}
            <h3 className="text-maroon-600 font-medium text-2xl border-b border-zinc-200 pb-3">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  {/*label*/}
                  <FormLabel className="text-zinc-800 font-medium text-sm dark:text-black">
                    Username
                  </FormLabel>
                  {/*input*/}
                  <FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  {/*input*/}
                  <FormControl>
                    <PhoneInput {...field} placeholder="Phone number" className="w-full" />
                  </FormControl>
                  {/*message*/}
                  <FormMessage />
                </FormItem>
        ...
        {step === 1 && (
          <>
            ...
            <div className="flex gap-4 border-b border-zinc-200 pb-3">
              {/*icon*/}
              <div
                className="size-[35px] rounded-full bg-maroon-600 flex items-center justify-center cursor-pointer"
                onClick={() => {
                  setStep(0);
                }}
              >
        ...
        >
          {isPending ? "Saving..." : step < steps.length - 1 ? "Next" : "Add Address"}
        </Button>
      </form>
    </Form>
  );
