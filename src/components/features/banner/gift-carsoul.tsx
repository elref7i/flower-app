    ...
    <div className='rounded-2xl'>
      <Carousel opts={{ align: "start", loop: false }} className="relative rounded-2xl overflow-hidden" setApi={setCarouselApi}>
        ...
        <CarouselContent>
          {slides.map((slide, i) => (
            <CarouselItem key={i} className='relative rounded-2xl overflow-hidden'>
              {/*Image*/}
              <Image src={slide.src} width={956} height={440} alt={slide.title} className='w-full h-[470px] object-cover' />
              {/*overlay*/}
        ...
        <div className="flex justify-between absolute bottom-5 right-6 z-10 bg-maroon-50  rounded-full shadow-md">
          <ChevronLeft className={`size-[30px] ${carouselApi?.canScrollPrev()
            ? " text-maroon-700 cursor-pointer"
            : " text-zinc-500 cursor-not-allowed"
            }`} onClick={() => carouselApi?.scrollPrev()} />
          <ChevronRight className={`size-[30px] ${carouselApi?.canScrollNext()
            ? " text-maroon-700 cursor-pointer"
            : " text-zinc-500 cursor-not-allowed"
        ...
        <div className="absolute top-6 right-6 z-10 flex gap-2">
          {slides.map((_, index) => (
            <div
              ...
              className={`h-2.5 rounded-[46.6px] transition-all duration-300 ${current === index ? 'bg-maroon-600 w-9' : 'bg-maroon-50 w-2.5'
                }`}
            ></div>
          ))}
        </div>
